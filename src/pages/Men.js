...
function Men() {
  ...
  return (
    <div className="mx-auto h-15 product-card-container">
      <Container fluid>
        <Row className="text">
          <h2 className="text-2xl font-bold text-center bg-men">Men</h2>
        </Row>
        <Row>
          {products.map((product, index) => {
            if (products.length === index + 1) {
              return (
                <Col ref={lastProductElementRef} key={product._id} md={4}  onClick={navigate(`/products/${product._id}`)} className="mb-4" >
                  {product.image && (
